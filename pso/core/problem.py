...
class Problem:
    ...
    def evaluate_gBest(self, particles:Particles):
        if self.objective_type == "min":
            ...
            if (self.gBest is None or 
                pBest_min <= self.gBest):
                self.gBest = pBest_min
                self.gBest_chromosome = pBest_min_particle.chromosome
        else:
            ...
            if (self.gBest is None or 
                pBest_max >= self.gBest):
                self.gBest = pBest_max
                self.gBest_chromosome = pBest_max_particle.chromosome
